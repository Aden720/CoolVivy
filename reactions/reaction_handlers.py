...
class PaginatedSelect(discord.ui.View):
    ...
    def update_select_menu(self):
        ...
        if not page_options:
            ...
            select = discord.ui.Select(
                placeholder=f"Select emojis ({remaining} slots left)",
                options=page_options,
                max_values=min(len(page_options), remaining)
            )

        async def select_callback(interaction: discord.Interaction):
            new_emojis = [
                next((opt.emoji for opt in select.options if opt.value == value), None)
